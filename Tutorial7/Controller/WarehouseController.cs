...
public class WarehouseController : ControllerBase
{
    ...
    {
        // ---------------------------------------------------------------------------------------------------
        // Tak naprawdę większość poniższego kodu powinno być wydzielone do osobnego repo, ewentualnie jakichs serwisow itp.,
        // jednak dla przyspieszenia pracy (i dlatego, ze musze zrobic tylko jeden endpoint) umieszczam calosc tutaj.
        // ---------------------------------------------------------------------------------------------------
        ...
        try
        {
            ...
            var productExistsCommand = new SqlCommand("SELECT 1 FROM Product WHERE IdProduct = @IdProduct", connection, transaction);
            productExistsCommand.Parameters.AddWithValue("@IdProduct", createProductWarehouseDto.IdProduct);

            await using (var productReader = await productExistsCommand.ExecuteReaderAsync())
            {
            ...
            var warehouseExistsCommand = new SqlCommand("SELECT 1 FROM Warehouse WHERE IdWarehouse = @IdWarehouse", connection, transaction);
            warehouseExistsCommand.Parameters.AddWithValue("@IdWarehouse", createProductWarehouseDto.IdWarehouse);

            await using (var warehouseReader = await warehouseExistsCommand.ExecuteReaderAsync())
            {
                if (!await warehouseReader.ReadAsync())
                {
            ...
            var orderCount = (int)await orderCommand.ExecuteScalarAsync();
            ...
            var fulfilledCommand = new SqlCommand(
            ...
            var fulfilledCount = (int)await fulfilledCommand.ExecuteScalarAsync();
            ...
            await using (var reader = await updateOrderCommand.ExecuteReaderAsync())
            {
            ...
            var productPrice = (decimal)await productPriceCommand.ExecuteScalarAsync();
            ...
            var generatedId = Convert.ToInt32(await insertCommand.ExecuteScalarAsync());
